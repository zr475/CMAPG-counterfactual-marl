...
class SequentialKeyLockEnv(gym.Env):
    ...
    def _reset_internal(self):
        ...
        self.step_credit = np.zeros(self.n_agents)  # Per-step contributions
    ...
    def step(self, actions: Dict[str, int]) -> Tuple[Dict, float, bool, bool, Dict]:
        ...
        self.step_credit = np.zeros(self.n_agents)  # Reset per-step credit
        ...
        for i in range(self.n_agents):
            ...
            if action == 3 and self.next_button < self.n_agents:  # Press
                ...
                if dist <= self.press_radius:
                    ...
                    self.step_credit[i] = 1.0  # Per-step contribution
                    self.next_button += 1
    ...
    def get_step_credit(self) -> np.ndarray:
        """Return per-step individual contributions (0 or 1 for each agent)."""
        return self.step_credit.copy()
