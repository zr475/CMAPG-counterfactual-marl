"""Main training script for SVPG and baseline algorithms.

Usage:
    python train.py --algo svpg --env key_lock --n_agents 5 --seed 42
    python train.py --algo mappo --env key_lock --n_agents 5 --seed 42
    python train.py --algo svpg --env transport --n_agents 3 --seed 42
    python train.py --algo svpg --env mpe --scenario simple_spread_v3 --seed 42
"""
...
from experiments.algorithms import SVPG, CMAPG, COMA, MAPPO, QMIX
from experiments.utils import Logger, evaluate, compute_credit_assignment_accuracy


def parse_args():
    parser = argparse.ArgumentParser(description="MARL Training with SVPG")

    # Experiment
    parser.add_argument("--algo", type=str, default="svpg",
                        choices=["svpg", "cmapg", "coma", "mappo", "qmix"])
    ...
    parser.add_argument("--K", type=int, default=20,
                        help="Number of Monte Carlo permutations for Shapley estimation")
...
def create_algorithm(algo_name: str, env, args):
    ...
    if hasattr(env, "get_state"):
        ...
        state_dim = obs_dim * args.n_agents
...
def train_policy_gradient(algo, env_ctor, args, logger: Logger):
    ...
    while total_steps < args.total_steps:
        ...
        if total_steps % args.eval_freq == 0:
            ...
            logger.save()  # save metrics after each eval for monitoring
        ...
        if total_steps % args.log_freq == 0 and len(list(logger.metrics.keys())) > 0:
            ...
            logger.save()  # periodic save for monitoring
...
def train_qmix(algo: QMIX, env_ctor, args, logger: Logger):
    ...
    while total_steps < args.total_steps:
        ...
        if total_steps % args.log_freq == 0:
            print(f"Step {total_steps:7d} | epsilon: {algo.epsilon:.3f}")
            logger.save()
...
def main():
    ...
    if hasattr(algo, "actors"):
        n_params = sum(p.numel() for actor in algo.actors for p in actor.parameters())
        print(f"Actor parameters: {n_params}")
    if hasattr(algo, "K"):
        print(f"Shapley MC permutations K: {algo.K}")
    ...
    try:
        if args.algo in ["svpg", "cmapg", "coma", "mappo"]:
